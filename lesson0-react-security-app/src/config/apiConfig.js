...
/*
    ajax를 위한 axios interceptor 를 구성
    axios interceptor는 프론트 리액트의 모든 request 와 response 를 
    가로채어서 공통 업무를 적용하는 역할을 함.
    공통 업무란 request 시에 request message header에 content-type : application/json 을 명시하고
    Authorization name 에 JWT 토큰 정보를 저장해서 함께 전송하는 공통 업무
    또한 response 시에 401(인증에러) 또는 403(인가 or 권한에러) 에러 발생 시 
...
api.interceptors.request.use(
  (config) => {
    ...
    //http request header 에 Authorization 이름으로 JWT 를 추가
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (status === 401 || status === 403) {
      ...
      window.location.href = "/login"; //인증/인가 오류 시에는 로그인 페이지로 이동시킨다
    }
