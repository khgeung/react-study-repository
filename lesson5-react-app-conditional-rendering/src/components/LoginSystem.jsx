...
function LoginSystem() {
  ...
  const handleLogin = () => {
    ...
    if (name) {
      setUsername(name); // React state hook 을 이용해 username 을 할당하여 리렌더링되어 보여짐
      setIsLoggedIn(true); // React state hook 을 이용해 loggedIn 할당하여 리렌더링되어 보여짐
    }
  };
  //로그아웃 처리 함수
  const handleLogout = () => {
    setUsername(""); // state hook 함수 이용해 정보 변경
    setIsLoggedIn(false); //state hook 함수 이용
  };
  ...
  return (
    <div className="login-system">
      <h2>로그인 시스템</h2>
      {/* 삼항 연산자 : isLoggedIn state 값에 따라 true 이면 로그인 화면 렌더링과 false 이면 비로그인 화면 렌더링을 구분해 보여준다 */}
      {isLoggedIn ? (
