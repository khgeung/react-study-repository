...
const App = () => {
  // useState hook 으로 ajax 요청 중인지를 저장
  // loading : 상태 변수, setloading : 상태 함수
  const [loading, setLoading] = useState(false); // API 서버 요청중일때는 요청중이라는 메시지 또는 이미지를 제공해야 한다.
  ...
  //의존성 배열은 빈배열을 할당해서 마운트 시점에 상품 리스트 데이터 로딩이 한번만 수행되도록 한다
  useEffect(() => {
  ...
  const loadProducts = async () => {
    setLoading(true); //현재 로딩중임을 알림
    try {
      //axios : 요청에 대한 응답이 마무리될때까지 기다리기
      const response = await axios.get(API_BASE_URL);
      setProducts(response.data); //useState hook 에서 제공한 (위에 정의) setProducts 상태 함수를 실행해 화면에 리렌더링 -> API Server 에서 받아온 상품 리스트를 화면에 보여지게 한다
  ...
  const addProduct = async (productData) => {
    ...
    try {
      ...
      console.log("상품등록 OK", response);
      /*상품 등록이 성공되면 상품 리스트에 추가 상품을 새 배열 마지막에 추가해 state에 할당 -> 리렌더링 -> ProductList component에 새로운 props(상품리스트) 전달 -> 리스트 갱신
       */
  ...
  return (
    <div className="app">
      ...
      {loading ? (
        ...
        //axios로 api server에서 받아온 상품 리스트를 props로 전달한다
